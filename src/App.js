...
export default  class App extends Component {
	...
	handleChange = (e) => {
 	...
 	handleClickStart = () => {
	  	this.setState({numOfRound: this.state.numOfRound+1});

	}

  	handleClickSubmit = () => {
	  	console.log(chosenArtist[this.state.numOfRound-1]);
	  	this.setState({try: this.state.try+1});
	  	this.setState({valueAttr: ''});
	  	if (this.state.valueAttr.toUpperCase()===chosenArtist[this.state.numOfRound-1].toUpperCase()){
	    	readData(this.state.numOfRound+1);
  ...
  render() {
			...
			<div id="App">
				...
				<button className="startBoxbutton" onClick={this.handleClickStart}>Click to start </button>
			</div>
		</div>
	    );
  	}
  	else if (this.state.numOfRound === 1 && this.props.first===1 ){
		readData(1);
		return <div></div>; //
  	}
  	else if (this.state.numOfRound < 6){
	    ...
	    return (
	    	<div id="startBox">
				...
				<button id="answerButton" onClick={this.handleClickSubmit} >Send</button>
...
function readData(numOfRound){
	...
	return fetch(urlApi)
	  	...
	  	.then(function(data) {
			...
			ReactDOM.render(<App ans={answer} first={0}/>, document.getElementById('root'));//
			numOfRound+=1;
			return;  
		});
	}
